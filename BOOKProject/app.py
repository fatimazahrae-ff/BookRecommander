...
app.debug = True
app.config["SQLALCHEMY_DATABASE_URI"] = 'postgresql://postgres:0000@localhost/test'
...
def index():
    ...
    return render_template('index.html', user=current_user, userRatings=userRatings, books=books,
                           recommendations=recommendations, genres=genres)
...
def addCommande():
    data = request.get_json()
    Commande =Commande(id=data["ID"],book=data["book"], userID=data["userID"] , date_cmnd=["date_cmnd"] )
    queryCommande = Commande.query.filter_by(ID=data["ID"]).first()
    if not queryCommande:
...
@app.route('/database')
def database():
    bookData = pd.read_csv("C:/Users/HP WORKSTATION/Desktop/book/templates/database/newBooks.csv", header=0,
                           names=["bookID", "genre", "title"])
    for index, row in bookData.iterrows():
        book = Book(bookID=row['bookID'], genre=row['genre'], title=row['title'])
        queryUser = User.query.filter_by(id=User.username, username=User.username).first()
        db.session.add(book)
        db.session.commit()

    userData = pd.read_csv('C:/Users/HP WORKSTATION/Desktop/book/templates/databas/ratings.csv', header=0,
                           names=["userID", "bookID", "bookRating"])
    ...
    userData = pd.read_csv('C:/Users/HP WORKSTATION/Desktop/book/templates/database/ratings.csv', header=0,
                           names=["userID", "bookID", "bookRating"])
    subUserData = userData[:10000]
    prevUserID = -1
    for index, row in subUserData.iterrows():
